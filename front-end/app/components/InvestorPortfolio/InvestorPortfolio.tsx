...
> = ({
    ...
    userAddress,
        ...
        return (
            <div className="max-w-lg rounded-lg ml-20 mb-10 overflow-hidden bg-white dark:bg-gray-800 border border-gray-300 transform transition duration-300 hover:scale-105 hover:shadow-xl">
                <img
                ...
                <div className="p-4">
                    <div className="flex items-center mb-2">
                        <span className="font-bold text-2xl dark:text-gray-200 text-gray-600">{title}</span>
                    </div>
                    ...
                    <div className="flex flex-col justify-between mb-2">
                        <div className="flex items-center justify-between dark:text-gray-200 text-gray-600 mb-1">
                            <span>Value:</span>
                            <span className="dark:text-gray-200 text-gray-600 text-lg">${value}</span>
                        </div>
                        <div className="flex items-center justify-between dark:text-gray-200 text-gray-600 mb-1">
                            <span>Earned Yield:</span>
                            <span className="dark:text-gray-200 text-gray-600 text-lg">${earnedYield}</span>
                        </div>
                        <div className="flex items-center justify-between dark:text-gray-200 text-gray-600 mb-1">
                            <span>Yield Percentage:</span>
                            <span className="dark:text-gray-200 text-gray-600 text-lg">{apy}%</span>
                        </div>
                        <div className="flex items-center justify-between dark:text-gray-200 text-gray-600 mb-1">
                            <span>Holding Tokens:</span>
                            <span className="dark:text-gray-200 text-gray-600 text-lg">{holdingTokens}</span>
                        </div>
                        <hr className="my-1 border-gray-300" />
                    </div>

                    <button
                        className="w-full text-white font-bold py-2 px-2 rounded bg-blue-500 dark:bg-blue-600 hover:bg-blue-600 dark:hover:bg-blue-700 transition duration-300"
...
const PortfolioOverview: React.FC = () => {
        ...
        try {
            ...
            const transformedData = data.map((property: PropertyCardProps) => {
                const userData = property.userData.find(
                    (user) => user.userAddress === account?.address
                );
                return {
                    ...property,
    ...
    return (
        <div className="flex min-h-screen flex-col font-serif items-center justify-center bg-gradient-to-b from-purple-200 to-blue-400 dark:from-purple-600 dark:to-blue-800 mt-14 pt-8">
            {!isConnected || isLoading ? (
                <div className="flex items-center justify-center h-full text-center bg-gray-300 dark:bg-gray-800 text-2xl font-bold text-purple-400 dark:text-purple-600 p-10 rounded-lg shadow-md ">
                    Please Connect Your Wallet To View The Portfolio
                </div>
            ) : (
                <>
                    ...
                    {aggregatedData && (
                        <div className="flex flex-col lg:flex-row gap-14 border border-white  w-full max-w-6xl bg-white dark:bg-gray-800 shadow-lg rounded-lg mt-4 pl-14 pr-14 mb-10 z-0">
